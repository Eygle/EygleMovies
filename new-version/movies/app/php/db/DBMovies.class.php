...
class DBMovies extends DAO {
	...
	public function getGenres() {
        ...
        public function getMovies($from, $nbr, $genre, $search) {
            ...
            $stmt = $this->pdo->prepare("SELECT m.id, title, YEAR(releaseDate) AS year, poster, localPoster"
                    ...
                    . " allocineId > 0 AND allocineId IS NOT NULL AND trash = 0"
	...
	public function getUncompleteMovies() {
		$stmt = $this->pdo->prepare('SELECT * FROM movies WHERE (allocineId IS NULL OR allocineId < 0) AND multipleChoices != 1 AND trash = 0');
		$stmt->execute();
		return $stmt->fetchAll(PDO::FETCH_ASSOC);
	}

    public function getTotalUncompleteMovies() {
        $stmt = $this->pdo->prepare('SELECT COUNT(*) AS tot FROM movies WHERE (allocineId IS NULL OR allocineId < 0) AND multipleChoices != 1 AND trash = 0');
        $stmt->execute();
        $res = $stmt->fetch(PDO::FETCH_ASSOC);
        return intval($res['tot']);
    }
