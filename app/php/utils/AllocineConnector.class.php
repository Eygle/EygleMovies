		...
		$res->poster = $html->find('div.poster', 0)->find('img', 0)->src;
		 ...
		 $table = $html->find('div.expendTable', 0);
                 if ($table) {
                     ...
                     $values = $table->find('tr');
                     
                     foreach ($values as $v) {
                         $tmp = $v->find('div');
                         if (isset($tmp[0]) && $tmp[0]->innertext == "Titre original") {
                             $res->originalTitle = $v->find('td', 0)->innertext;
                             break;
                         }
                     }
                 }
                 
                $res->title = htmlspecialchars_decode(html_entity_decode($res->title, ENT_QUOTES | ENT_HTML5));
                if ($res->originalTitle) {
                    $res->originalTitle = htmlspecialchars_decode(html_entity_decode($res->originalTitle, ENT_QUOTES | ENT_HTML5));
                }
